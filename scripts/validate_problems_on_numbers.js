...
const filePath = path.join(__dirname, '../server/data/rs_agrawal/problems_on_numbers.json');
...
if (data.questions.length !== 133) {
  console.error(`ERROR: Expected 133 questions, found ${data.questions.length}`);
  errors++;
}
...
data.questions.forEach((q, idx) => {
  ...
  // Check for OCR artifacts like 'th of' or broken tags
  if (/th of/.test(q.question) || /rd of/.test(q.question)) {
    console.warn(`WARNING: Q${qNum} might contain raw fraction OCR artifact: "${q.question}"`);
    warnings++;
  }
});
