...
const filePath = path.join(__dirname, '../server/data/rs_agrawal/problems_on_ages.json');
...
if (data.questions.length !== 57) {
  console.error(`ERROR: Expected 57 questions, found ${data.questions.length}`);
  errors++;
}
...
data.questions.forEach((q, idx) => {
  ...
  // Check for broken OCR artifacts
  if (/\b\d+\s+\d+\s+times\b/.test(q.question) || /\b\d+\s+th\s+\d+\b/.test(q.question)) {
